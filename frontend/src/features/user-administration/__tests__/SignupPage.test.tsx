...
describe('SignupPage', () => {
  ...
  it('shows error when passwords do not match', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Nome Completo/i), { target: { value: 'Test User', name: 'full_name' } });
    fireEvent.change(screen.getByLabelText(/E-mail/i), { target: { value: 'test@test.com', name: 'email' } });
    fireEvent.change(screen.getByLabelText(/Usuário/i), { target: { value: 'testuser', name: 'username' } });
    fireEvent.change(screen.getByLabelText('Senha'), { target: { value: 'password123!', name: 'password' } });
    fireEvent.change(screen.getByLabelText(/Confirmar Senha/i), { target: { value: 'different!', name: 'confirm_password' } });
  ...
  it('shows error when password is too short', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/E-mail/i), { target: { value: 'test@test.com', name: 'email' } });
    fireEvent.change(screen.getByLabelText(/Usuário/i), { target: { value: 'testuser', name: 'username' } });
    fireEvent.change(screen.getByLabelText('Senha'), { target: { value: 'short', name: 'password' } });
    fireEvent.change(screen.getByLabelText(/Confirmar Senha/i), { target: { value: 'short', name: 'confirm_password' } });
    fireEvent.click(screen.getByRole('button', { name: /Cadastrar/i }));

    expect(screen.getByText(/A senha deve ter pelo menos 8 caracteres/i)).toBeDefined();
  });
  ...
  it('navigates to login with success message on successful signup', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/E-mail/i), { target: { value: 'test@test.com', name: 'email' } });
    fireEvent.change(screen.getByLabelText(/Usuário/i), { target: { value: 'testuser', name: 'username' } });
    fireEvent.change(screen.getByLabelText('Senha'), { target: { value: 'validpassword1!', name: 'password' } });
    fireEvent.change(screen.getByLabelText(/Confirmar Senha/i), { target: { value: 'validpassword1!', name: 'confirm_password' } });
    fireEvent.click(screen.getByRole('button', { name: /Cadastrar/i }));
  ...
  it('shows array validation error detail from API on failure', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Erro de validação: field required, invalid email/i)).toBeDefined();
    });
  });
  ...
  it('shows generic error message when no detail in response', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Ocorreu um erro ao realizar o cadastro/i)).toBeDefined();
    });
  });
  ...
  it('shows loading state while submitting', async () => {
    ...
    expect(screen.getByRole('button', { name: /Cadastrando.../i })).toBeDefined();
