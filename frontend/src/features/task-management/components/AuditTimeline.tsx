  ...
  if (!history || history.length === 0) {
    return (
      <div className={styles.timelineContainer}>
        ...
        <p className={styles.emptyState}>No changes recorded for this task yet.</p>
      </div>
    );
  }
        ...
        {history.map((entry) => (
          <div key={entry.id} className={styles.timelineItem}>
            ...
            <div className={styles.itemHeader}>
              ...
              <span className={styles.timestamp}>{formatDate(entry.timestamp)}</span>
            </div>
            <div className={styles.changeDetails}>
              <span className={styles.fieldName}>{formatFieldName(entry.field_name)}</span>
              <div className={styles.valueChange}>
                <span className={styles.oldValue}>{formatValue(entry.old_value)}</span>
                <span className={styles.arrow}>→</span>
                <span className={styles.newValue}>{formatValue(entry.new_value)}</span>
              </div>
            </div>
          </div>
        ))}
